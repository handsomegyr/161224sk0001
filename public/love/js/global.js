...
　　}
    ...
    function lottery() {
        if (lottery_num >= lottery_limit) {
			...
			showErrorTips('你的机会已经用完！<br>凌晨12点你将获得全新3次机会。<br>更多精品请关注我们的官方微信：yooxyooxyoox');
			$('.luck_join_text').text('').text('立即抽奖');
            return false;
        }
		//修改文字
		$('.luck_join_text').text('').text('正在派奖')
		.delay('2000')
		.queue(function(){
				...
				$.ajax({
					...
					success : function (res) {
						...
						if (res.success) {
							...
							$('.luck_join_text').text('').text('立即抽奖');
							$('.luck_join').on('click', lotteryAgain);
