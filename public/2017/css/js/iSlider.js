...
function openViewImg(imgSrcArrStr, imgIndex) {
    var viewImgIndex = Number(imgIndex);
    if (!document.getElementById('iSlider-wrapper')) {
        $("body").append('<div id="iSlider-wrapper" style="display:none;" ></div>');
        $("body").append('<div id="imgView_head"  style="display:block;  text-align:center; font-size:16px; background-color:black; height:50px; line-height:50px; width:100%; position:absolute; bottom:0; z-index:1000; color:white; font-size:18px;  " > '
            + '<div onclick="closeViewImg();" style="position:absolute; top:7px;  width:80px; text-align:left;" ><img style="height:36px; margin-left:10px; " src="/img/mobile/iconfont-miaojiesellerarrowleft.png" /></div><span id="currentIndex"></span>/<span id="span_imgCount"></span> <img id="img_delete" onclick="deleteOptPageImg()" style="display:none; position:absolute; height:24px; right:10px; top:13px;" src="/img/mobile/iconfont-iconfontdelete.png" />'
        + '</div>');
    }
 
    $("#imgView_head").show();
    $("#iSlider-wrapper").show();
    if (document.title == "客户详情") {
        $("#img_delete").hide();
    }

    var imgSrcArr = imgSrcArrStr.split(",");
    $("#span_imgCount").text(imgSrcArr.length);

    var currentIndexElement = $("#currentIndex");
    currentIndexElement.text(viewImgIndex + 1);

    var list = []
    for (var i = 0; i < imgSrcArr.length; i++) {
        list.push({ content: imgSrcArr[i] });
    }

    S = new iSlider({
        dom: document.getElementById('iSlider-wrapper'),
        data: list,
        plugins: [['zoompic', { zoomFactor: 1 }]],
        onslidechanged: function (index) {
            currentIndexElement.text(index + 1);
        }
    });

    S.slideTo(viewImgIndex);
}
